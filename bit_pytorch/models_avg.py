...
class CustomConv2dFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if zero_ratio > 0:
            # 각 채널의 그래디언트 합계 계산
            channel_sum_values = abs_grad_output.view(batch_size, num_channels, -1).sum(dim=2)
            channels_sorted_by_sum = torch.argsort(channel_sum_values, dim=1, descending=False)  # 오름차순 정렬

            # zero_ratio에 따라 하위 채널을 선택하여 0으로 설정
            num_channels_to_zero = int(num_channels * zero_ratio)
            channels_to_zero = channels_sorted_by_sum[:, :num_channels_to_zero]
        ...
        # 각 배치에 대해 동일하게 처리
        channel_sum_values = abs_grad_output.view(batch_size, num_channels, -1).sum(dim=2)
        channel_sum_values = channel_sum_values.sum(dim=0)
        _, channels_sorted_by_sum = torch.sort(channel_sum_values)  # 오름차순 정렬
