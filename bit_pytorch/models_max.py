...
class CustomConv2dFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        # # 각 배치에 대해 독립적으로 처리
        # if zero_ratio > 0:
        #     # 각 배치의 모든 채널에 대해 최대값을 구하고 정렬
        #     channel_max_values = grad_output.view(batch_size, num_channels, -1).max(dim=2).values
        #     channels_sorted_by_max = torch.argsort(channel_max_values, dim=1, descending=False)

        #     # zero_ratio에 따라 상위 채널을 선택하여 0으로 설정
        #     num_channels_to_zero = int(num_channels * zero_ratio)
        #     channels_to_zero = channels_sorted_by_max[:, :num_channels_to_zero]

        #     # 배치 차원을 확장하여 인덱스를 맞춘 후 0으로 설정
        #     batch_indices = torch.arange(batch_size, device=grad_output.device).view(-1, 1).expand(-1, num_channels_to_zero)
        #     grad_output[batch_indices, channels_to_zero, :, :] = 0

        #     zero_count_tensor = (grad_output == 0).float().mean(dim=[1,2,3])
        #     all_zero_ratio.append(zero_count_tensor.mean().item())
        
        
        # 각 채널의 최대값을 기준으로 정렬
        channel_max_values = abs_grad_output.view(batch_size, num_channels, -1).max(dim=2).values
        batch_max_values = channel_max_values.max(dim=0).values
        _, channels_sorted_by_max = torch.sort(batch_max_values)
  ...
  def __init__(self, block_units, width_factor, head_size=21843, zero_head=False):
    ...
    self.body = nn.Sequential(OrderedDict([
        ('block1', nn.Sequential(OrderedDict(
            [('unit01', PreActBottleneck(cin=64*wf, cout=256*wf, cmid=64*wf, threshold=0, zero_ratio=0.5))] +
            [(f'unit{i:02d}', PreActBottleneck(cin=256*wf, cout=256*wf, cmid=64*wf, threshold=0, zero_ratio=0.5)) for i in range(2, block_units[0] + 1)],
        ))),
        ('block2', nn.Sequential(OrderedDict(
            [('unit01', PreActBottleneck(cin=256*wf, cout=512*wf, cmid=128*wf, threshold=0, stride=2, zero_ratio=0.5))] +
            [(f'unit{i:02d}', PreActBottleneck(cin=512*wf, cout=512*wf, cmid=128*wf, threshold=0, zero_ratio=0.5)) for i in range(2, block_units[1] + 1)],
        ))),
        ('block3', nn.Sequential(OrderedDict(
            [('unit01', PreActBottleneck(cin=512*wf, cout=1024*wf, cmid=256*wf, threshold=0, stride=2, zero_ratio=0.5))] +
            [(f'unit{i:02d}', PreActBottleneck(cin=1024*wf, cout=1024*wf, threshold=0, cmid=256*wf, zero_ratio=0.5)) for i in range(2, block_units[2] + 1)],
        ))),
        ('block4', nn.Sequential(OrderedDict(
            [('unit01', PreActBottleneck(cin=1024*wf, cout=2048*wf, cmid=512*wf, threshold=0, stride=2, zero_ratio=0.5))] +
            [(f'unit{i:02d}', PreActBottleneck(cin=2048*wf, cout=2048*wf, cmid=512*wf, threshold=0, zero_ratio=0.5)) for i in range(2, block_units[3] + 1)],
        ))),
    ]))
