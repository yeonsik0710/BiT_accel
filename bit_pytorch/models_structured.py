...
# batch마다 몇 %의 channel 전체를 0으로 보냄
...
class CustomConv2dFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if zero_ratio > 0:
            ...
            # 랜덤한 인덱스 생성
            rand_indices = torch.rand(batch_size, num_channels, device=grad_output.device).argsort(dim=1)
            zero_indices = rand_indices[:, :num_channels_to_zero]

            # 배치 인덱스 생성
            batch_indices = torch.arange(batch_size, device=grad_output.device).view(-1, 1).expand(-1, num_channels_to_zero)

            # grad_output에서 특정 채널을 0으로 설정
            grad_output[batch_indices, zero_indices, :, :] = 0

            # zero_count_tensor 계산
        ...
        # 채널의 50%를 랜덤하게 0으로 설정
        if zero_ratio > 0:
            _, num_channels, _, _ = grad_output.shape
            num_channels_to_zero = int(num_channels * zero_ratio)
            zero_count_tensor = torch.zeros(batch_size, device=grad_output.device)
  ...
  def __init__(self, block_units, width_factor, head_size=21843, zero_head=False):
    ...
    self.body = nn.Sequential(OrderedDict([
        ...
        ('block3', nn.Sequential(OrderedDict(
            [('unit01', PreActBottleneck(cin=512*wf, cout=1024*wf, cmid=256*wf, threshold=0, stride=2, zero_ratio=0, name="block3"))] +
            [(f'unit{i:02d}', PreActBottleneck(cin=1024*wf, cout=1024*wf, threshold=0, cmid=256*wf, zero_ratio=0, name="block3")) for i in range(2, block_units[2] + 1)],
        ))),
