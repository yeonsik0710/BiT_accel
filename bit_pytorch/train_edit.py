...
def main(args):
  ...
  with lb.Uninterrupt() as u:
    ...
    for x, y in recycle(train_loader):
      ...
      for module, grad_output in output_gradients.items():
          if module not in all_zero_ratio:
            all_zero_ratio[module] = torch.tensor([], device=grad_output.device)

          out_zero_elements = torch.sum(grad_output == 0).item()
          out_total_elements = torch.numel(grad_output)
          ...
          if step >= warmup_step:
            ...
            zero_ratio = torch.tensor([zero_ratio], device=all_zero_ratio[module].device)
            all_zero_ratio[module] = torch.cat([zero_ratio, all_zero_ratio[module]], dim=0)
            zero_ratio = zero_ratio.item()
          logger.info(f"Layer {module} - Output Gradient Zero Ratio: {zero_ratio:.5f}%, out_zero_elements : {out_zero_elements}, out_total_elements : {out_total_elements}")
      logger.flush()
      ...
      if (step == 9000) and (accum_steps == 1):
        grad_out_dist = models.get_output_gradient() # 4000~6000 step에서 max value 가져옴

        for layer_id, grad_out in grad_out_dist.items(): # 딕셔너리에서 하나씩 꺼내와서 npz 파일로 저장
          save_path = pjoin(args.logdir, args.name, 'grad_out_9000', f"grad_out_{layer_id}_9000.npz")
          np.savez(save_path, data=grad_out.cpu().numpy())
        logger.info("npz file 저장 완료.")
      ...
      if (step == warmup_step) and (accum_steps == 0):
          ...
          for layer_id in accum_c_max_values:
              ...
              mean_value = torch.mean(accum_c_max_values[layer_id])
              std_value = torch.std(accum_c_max_values[layer_id])

              # 각 레이어의 T 값을 설정
              for module in model.modules():
                  if isinstance(module, models.CustomConv2d) and module.layer_id == layer_id:
                      module.T = mean_value + std_value
                      logger.info(f"Set T for layer {layer_id} to {module.T}") # 모니터링
    ...
    for layer_id, zero_ratio in all_zero_ratio.items():
      avg_zero_ratio = torch.mean(zero_ratio)
