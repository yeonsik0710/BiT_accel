...
def main(args):
  ...
  # 저장 경로 설정
  grad_save_path = pjoin(args.logdir, args.name)
  

  with lb.Uninterrupt() as u:

    for step, (x, y) in enumerate(recycle(train_loader)):
      ...
      # 학습 루프 내에서 100 step마다 gradient summation 값을 파일로 저장
      if (step == 499) or (step == 2999) or (step == 5999) or (step == 8999) or (step == 9999):
          grad_block1, grad_block2, grad_block3, grad_block4 = models.get_grad()
          grad_block1 = np.array(grad_block1, dtype=object)
          grad_block2 = np.array(grad_block2, dtype=object)
          grad_block3 = np.array(grad_block3, dtype=object)
          grad_block4 = np.array(grad_block4, dtype=object)

          # log 이름 바꾸기
          grad_save_path_block1 = pjoin(grad_save_path, f"gradient_block1_{step}.npz")
          grad_save_path_block2 = pjoin(grad_save_path, f"gradient_block2_{step}.npz")
          grad_save_path_block3 = pjoin(grad_save_path, f"gradient_block3_{step}.npz")
          grad_save_path_block4 = pjoin(grad_save_path, f"gradient_block4_{step}.npz")

          # 각 블록을 개별 파일로 저장
          np.savez(grad_save_path_block1, block1=grad_block1)
          np.savez(grad_save_path_block2, block2=grad_block2)
          np.savez(grad_save_path_block3, block3=grad_block3)
          np.savez(grad_save_path_block4, block4=grad_block4)
